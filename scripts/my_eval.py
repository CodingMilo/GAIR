...
def get_rag_engine(client, config):
    if not hasattr(sys, "_gair_rag_engine"):
...
def evaluate_question(
    ...
    rag = get_rag_engine(client, config)
    context = rag.get_context(question, k=2)

    # 2. Prepare Prompt with Context
    system_prompt = config.get('system_prompt', "You are a Reliability Engineering expert.")
    
    user_content = f"CONTEXT FROM DOCUMENTATION:\n{context}\n\nQUESTION:\n{question}" if context else question

    prompt = """
    You will be given a CRE exam question and potentially some context. 
    Use the context if relevant. Do not reason and choose the correct answer(s) directly. 
    At the end of your response, start a new line and output your answer as a JSON object with the following format:
    {
